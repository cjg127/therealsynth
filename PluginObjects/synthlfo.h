...
struct SynthLFOParameters
{
	...
	LFOWaveform lfoShape = LFOWaveform::kTriangle;
...
public:
	...
	{ 
		rampInc = 1.0 / (msecToSamples(sampleRate, parameters->lfoRamp_mSec));
		rampMult = 0.0;
		lfoTimer.resetTimer();
		lfoTimer.setTargetValueInSamples(msecToSamples(sampleRate, parameters->lfoDelay_mSec));
